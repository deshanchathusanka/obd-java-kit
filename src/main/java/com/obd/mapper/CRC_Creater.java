 ...
 */
public class CRC_Creater {
    private final int FCS_START = 0xFFFF;
    private final int FCS_FINAL = 0xF0B8;

    private final int[] FCS_TAB = {
    ...
    public int getFCS(int fcs, int src){
        int xor = 0x0000;
        int iresult = 0x0000;
        xor = fcs;
        xor ^=src;
        iresult=(fcs>>8)^FCS_TAB[xor & 0x00FF];
        return iresult;

    }

    public int makeCRC(byte[] pscr,int length){
        int crc = FCS_START;
        for(int i=0;i<length;i++){
            crc=getFCS(crc,pscr[i]);
        }
        crc ^= FCS_START;
    ...
    public  String getCRC(String rawData){
        byte[] b = hexStringToByteArray(rawData);
        String s = Integer.toHexString(makeCRC(b,b.length));
        char[] c = s.toCharArray();
        char[] finalCharArray = new char[4];
        finalCharArray[0] = c[2];
        finalCharArray[1] = c[3];
        finalCharArray[2] = c[0];
        finalCharArray[3] = c[1];
